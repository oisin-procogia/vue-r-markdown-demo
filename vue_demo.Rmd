---
title: "Filtering the NBA's Top-20 Scorers with Vue.js"
...
top_twenty_players <- players %>%
  top_n(20, pts) %>% 
  arrange(desc(pts))

json_players <- toJSON(top_twenty_players, pretty=TRUE)

cat("<script> var topTwenty=",
    json_players,
    "</script>")
```
...
var vue = new Vue({
    ...
    data: {
      ...
      slider: {
        min: topTwenty.reduce((prev, curr) => prev.age < curr.age ? prev : curr).age,
        max: topTwenty.reduce((prev, curr) => prev.age > curr.age ? prev : curr).age
      } 
    },
  methods: {
